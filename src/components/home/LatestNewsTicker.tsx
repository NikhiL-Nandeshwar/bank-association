...
function NewsCard({ item, compact = false }: { item: NewsItem; compact?: boolean }) {
  ...
  return (
    <article
      className={`group flex h-full min-h-[190px] w-[320px] shrink-0 flex-col justify-between rounded-lg border ${styles.border} bg-white p-5 shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-lg sm:w-[380px] ${
        compact ? 'min-h-[210px] w-full sm:w-full' : ''
      }`}
    >
      <div>
        ...
        <p className="mt-2 line-clamp-3 text-md leading-6 text-slate-700">{item.summary}</p>
      </div>
      ...
      <div className="mt-5 flex items-center justify-between gap-3 border-t border-slate-100 pt-4 text-sm">
        ...
        <ArrowUpRight className="h-5 w-5 text-[#7A2E92] transition group-hover:translate-x-0.5 group-hover:-translate-y-0.5" aria-hidden="true" />
      </div>
    </article>
  );
}
